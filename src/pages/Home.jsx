...
export default function Home() {
  ...
  const onSubmit = (name) => {
    ...
    setTimeout(() => {
      ...
      // แสดงอิโมจิ overlay แค่ช่วงสั้น ๆ
    ...
    <div className="mx-auto flex min-h-screen max-w-7xl flex-col px-4 py-10 text-center">
      ...
      <div className="flex-grow flex flex-col items-center justify-center">
        ...
        >
          ...
          <p className="mt-1 text-slate-300">
            ป้อนชื่อของคุณเพื่อรับคำยินดีสุดพิเศษ
          </p>
        </motion.div>
        ...
        {/* แบนเนอร์ยินดี โผล่เมื่อมีผลลัพธ์ */}
      ...
      <footer className="mt-auto w-full border-t border-slate-200/20 pt-6">
        <p className="text-sm md:text-base text-slate-300">
          © 2025{" "}
          <span className="font-bold text-slate-100">Congrats-Jkphng</span> ·
        ...
        <div className="mt-4 flex items-center justify-center gap-5">
          ...
          <a
            ...
            target="_blank"
            className="group inline-flex h-10 w-10 items-center justify-center rounded-full bg-brand-yt/15 ring-1 ring-brand-yt/30 transition-all hover:scale-105 hover:bg-brand-yt/25"
          >
            <Youtube className="h-5 w-5 text-brand-yt transition-transform group-hover:scale-110" />
          </a>
          <a
...
function FloatingCaps() {
  ...
  return (
    <div className="relative mt-2 h-2 w-full max-w-l z-[10]">
      <AnimatePresence>
        {items.map((it, idx) => (
          <motion.div
            key={idx}
